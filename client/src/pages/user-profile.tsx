...
export default function UserProfile() {
  ...
  const handleSave = async () => {
    try {
      const hasImages = (editData.profilePicture && editData.profilePicture.startsWith("data:")) ||
                        (editData.coverImage && editData.coverImage.startsWith("data:"));
      ...
      if (hasImages) {
        ...
        Object.keys(editData).forEach(key => {
          if (key !== "profilePicture" && key !== "coverImage" && editData[key]) {
            formData.append(key, editData[key]);
          }
        });
        
        // Convert base64 images to blobs and append
        if (editData.profilePicture && editData.profilePicture.startsWith("data:")) {
          const blob = await fetch(editData.profilePicture).then(r => r.blob());
          formData.append("profilePicture", blob, "profile.jpg");
        }
        
        if (editData.coverImage && editData.coverImage.startsWith("data:")) {
          const blob = await fetch(editData.coverImage).then(r => r.blob());
